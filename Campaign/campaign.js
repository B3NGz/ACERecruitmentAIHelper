...
import {
  ...
  getCampaignNames,
  createCampaign,
  processCampaignApplicants,
  processCampaignRequirementDocument,
  importCampaignApplicants,
...
function setCampaignLoadStatus(message = '') {
  const status = document.getElementById('campaign-load-status');
...
function setCampaignProcessingState(isProcessing) {
  const overlay = document.getElementById('campaign-processing-overlay');
  const button = document.getElementById('compare-with-applicants-btn');

  if (overlay) {
    overlay.style.display = isProcessing ? 'flex' : 'none';
    overlay.setAttribute('aria-hidden', String(!isProcessing));
  }

  if (button) {
    button.disabled = isProcessing;
    button.setAttribute('aria-busy', String(isProcessing));
  }
}

function getProcessingResultMessage(result) {
  if (typeof result === 'string' && result.trim()) return result.trim();
  if (result?.message) return String(result.message);
  if (result?.title) return String(result.title);
  return 'Applicants processed successfully.';
}
...
function parseRequirementData(value) {
...
function getRequirementValue(data, key) {
  return data?.[key] ?? data?.[key.charAt(0).toLowerCase() + key.slice(1)] ?? null;
}

function formatRequirementValue(value) {
  if (Array.isArray(value)) {
    return `<ul style="margin:0;padding-left:1.2rem;display:grid;gap:0.35rem;">${value
      .filter(item => item !== null && item !== undefined && item !== '')
      .map(item => `<li>${escapeHtml(item)}</li>`)
      .join('')}</ul>`;
  }

  return `<p style="margin:0;white-space:pre-wrap;line-height:1.55;">${escapeHtml(value)}</p>`;
}
...
function setupRequirementViewer(campaign) {
  const viewBtn = document.getElementById('view-requirements-btn');
  const availability = document.getElementById('requirement-availability');
  const modal = document.getElementById('requirement-viewer-modal');
  const content = document.getElementById('requirement-viewer-content');
  const title = document.getElementById('requirement-viewer-title');
  const closeButtons = document.querySelectorAll('#close-requirement-viewer, #close-requirement-viewer-footer');
  if (!viewBtn || !availability || !modal || !content || !title) return;

  const structuredData = parseRequirementData(campaign.requirementData) || parseRequirementData(campaign.requirementDocumentUrl);
  const documentUrl = isSafeDocumentUrl(campaign.requirementDocumentUrl)
    ? campaign.requirementDocumentUrl
    : null;
  const hasRequirement = Boolean(structuredData || documentUrl);

  viewBtn.disabled = !hasRequirement;
  availability.textContent = hasRequirement
    ? (campaign.requirementDocumentName || 'Requirement information is available.')
    : 'No requirement document or requirement data is available for this campaign.';
  ...
  const renderViewer = () => {
    title.textContent = campaign.requirementDocumentName || 'Campaign Requirements';
    ...
    if (structuredData && typeof structuredData === 'object' && !Array.isArray(structuredData)) {
      ...
      const populatedFields = fields
        .map(([key, label]) => [label, getRequirementValue(structuredData, key)])
        .filter(([, value]) => value !== null && value !== undefined && value !== '');

      content.innerHTML = populatedFields.length
        ? populatedFields.map(([label, value]) => `
            <section style="padding:1rem;border:1px solid var(--border-light);border-radius:12px;background:var(--bg-glass);">
              <h3 style="margin:0 0 0.5rem;color:var(--text-primary);font-size:0.9rem;">${escapeHtml(label)}</h3>
              <div style="color:var(--text-secondary);font-size:0.86rem;">${formatRequirementValue(value)}</div>
            </section>`).join('')
        : '<p style="margin:0;color:var(--text-muted);">Requirement data was returned, but it does not contain displayable requirement fields.</p>';
      return;
    }

    if (documentUrl) {
      content.innerHTML = `<iframe src="${escapeHtml(documentUrl)}" title="${escapeHtml(campaign.requirementDocumentName || 'Campaign requirement document')}" style="width:100%;min-height:560px;border:1px solid var(--border-light);border-radius:12px;background:white;"></iframe>`;
      return;
    }

    content.innerHTML = '<p style="margin:0;color:var(--text-muted);">Requirement information is not available.</p>';
  };
    ...
    list.innerHTML = campaigns.map(c => {
        ...
        return `
            <tr class="${rowClass}" data-status="${status}">
                ...
                <td>
                    <a href="campaign-details.html?id=${encodeURIComponent(c.id)}">
                        ${c.jobTitle}
                    </a>
                </td>
                ...
                <td>
                    <a
                        href="campaign-details.html?id=${encodeURIComponent(c.id)}"
...
async function renderCampaignList() {
  ...
  if (statusDropdown) {
    ...
    value === 'all'
        ...
        const filtered = statusFiltered.filter(campaign =>
          [campaign.jobTitle, campaign.clientName, campaign.status]
  ...
  if (searchInput) {
    ...
    selectedValue === 'all'
      ...
      const searched = filtered.filter(campaign =>
        [campaign.jobTitle, campaign.clientName, campaign.status]
...
function compareApplicantsWithCriteria(campaignId, criteriaText) {
  ...
  container.innerHTML = scores.map((app, index) => {
    ...
    return `<div style="display:flex;justify-content:space-between;align-items:center;padding:0.5rem 0;border-bottom:1px solid var(--border-light);">
      <span><strong>#${index + 1}</strong> <a href="../Applicants/applicant-details.html?id=${encodeURIComponent(app.id)}" style="color:var(--link-color);text-decoration:none;">${app.fullName}</a></span>
...
function setupPdfExtraction(campaignId) {
    ...
    saveBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            const campaign = await processCampaignRequirementDocument(campaignId, file);
    ...
    compareBtn.addEventListener("click", async () => {
        ...
        setCampaignProcessingState(true);

        try {
            const result = await processCampaignApplicants(campaignId);
            const message = getProcessingResultMessage(result);

            await renderCampaignDetails();
            showToast(message, "success");
        }
        catch (error) {
            console.error('Campaign applicant processing failed:', error);
            ...
            setCampaignProcessingState(false);
        }
...
async function renderCampaignDetails() {
  ...
  if (!id) {
    ...
    setCampaignLoadStatus('Choose a campaign to view its details.');
    return;
  }

  setCampaignLoadStatus('Loading campaign details…');
  ...
  try {
    ...
    document.getElementById('campaign-title').textContent = 'Unable to load campaign';
    setCampaignLoadStatus(
      isNetworkError
  ...
  if (!campaign) {
    ...
    setCampaignLoadStatus('The requested campaign is unavailable.');
    return;
  }
  ...
  document.getElementById('campaign-created').textContent = formatCampaignDate(campaign.createdDate);
  document.getElementById('campaign-sheet-name').textContent = campaign.googleSheetName || 'Not connected';
  document.getElementById('campaign-last-row-processed').textContent = Number.isFinite(campaign.lastRowProcessed)
    ? String(campaign.lastRowProcessed)
    : 'N/A';
  setCampaignLoadStatus('');
  ...
  setupRequirementViewer(campaign);
  ...
  if (importBtn && !importBtn.dataset.bound) {
    ...
    importBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        await importCampaignApplicants(campaign.id);
  ...
  const renderApplicants = () => {
    ...
    if (rows.length === 0) {
      ...
      applicantList.innerHTML = rows.map(({ applicant: a, assessment }) => {
        ...
        return `<tr>
          <td><a href="../Applicants/applicant-details.html?id=${encodeURIComponent(a.id)}" style="color:var(--link-color);text-decoration:none;">${a.fullName}</a></td>
          <td>${a.currentPosition}</td>
          <td>${a.yearsExperience} yrs</td>
          <td>${score}</td>
          <td><span class="badge ${recClass} recommendation-preview">${rec}</span></td>
          <td><a href="../Applicants/applicant-details.html?id=${encodeURIComponent(a.id)}" class="btn btn-secondary" style="padding:0.2rem 0.8rem;font-size:0.8rem;">View</a></td>
        </tr>`;
      }).join('');
    }
  };
