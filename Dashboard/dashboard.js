...
function requireAuthentication() {
  if (!localStorage.getItem('token')) {
    window.location.href = 'https://b3ngz.github.io/Login/login.html';
    return false;
  }
  return true;
}
...
function renderActiveCampaigns(db) {
  ...
  list.innerHTML = activeCampaigns.map(campaign => {
    ...
    return `<tr>
      <td><a href="https://b3ngz.github.io/Campaign/campaign-details.html?id=${campaign.id}" style="color:var(--link-color);text-decoration:none;">${campaign.jobTitle}</a></td>
...
async function renderTopCandidates() {
  ...
  list.innerHTML = candidates.slice(0, 5).map((candidate, index) => `<tr>
    ...
    <td><a href="https://b3ngz.github.io/Applicants/applicant-details.html?id=${candidate.id}" style="color:var(--link-color);text-decoration:none;">${candidate.fullName}</a></td>
