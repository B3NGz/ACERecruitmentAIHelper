  ...
  list.innerHTML = itemsToShow.map((c, index) => {
    ...
    return `
      <tr>
        ...
        <td><a href="https://b3ngz.github.io/Applicants/applicant-details.html?id=${c.id}" style="color:var(--link-color);text-decoration:none;">${c.fullName}</a></td>
        <td>${c.campaignName}</td>
        <td><strong>${c.score}</strong></td>
        <td><span class="badge ${recClass} recommendation-preview">${c.recommendation}</span></td>
        <td><a href="https://b3ngz.github.io/Applicants/applicant-details.html?id=${c.id}" class="btn btn-secondary" style="padding:0.2rem 0.8rem;font-size:0.8rem;">View</a></td>
      </tr>
    `;
  }).join('');
...
function setupCompareSelected() {
  ...
  btn.addEventListener('click', function() {
    ...
    window.location.href = `https://b3ngz.github.io/Applicants/applicant-details.html?compare=${ids.join(',')}`;
  });
}
