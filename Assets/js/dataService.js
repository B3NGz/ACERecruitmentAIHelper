...
function normalizeHrAssessment(assessment) {
  if (!assessment) return null;

  const assessedAt = normalizeDateValue(
...
export async function processCampaignRequirementDocument(campaignId, file) {
  const formData = new FormData();
  formData.append('file', file);

  const result = await apiFetch(
    `/api/RequirementDocument/${encodeURIComponent(campaignId)}/process`,
...
export async function importCampaignApplicants(campaignId) {
  const result = await apiFetch(
    `/api/ApplicantImport/${encodeURIComponent(campaignId)}`,
    { method: 'POST' }
  );
...
export async function getHrAssessmentsByApplicant(applicantId) {
  if (!applicantId) return [];

  const result = await apiFetch(
    `/api/HrAssessment/applicant/${encodeURIComponent(applicantId)}`
  );

  return unwrapCollection(result)
    .map(normalizeHrAssessment)
    .filter(Boolean);
}

function toHrAssessmentRequest(assessment = {}) {
  return {
    applicantID: assessment.applicantId ?? assessment.applicantID,
    campaignID: assessment.campaignId ?? assessment.campaignID,
    assessmentNotes: assessment.assessmentNotes ?? '',
    hrScore: assessment.hrScore,
    hrComment: assessment.hrComment ?? '',
...
export async function createHrAssessment(assessment) {
  const result = await apiFetch('/api/HrAssessment', {
    method: 'POST',
    body: JSON.stringify(toHrAssessmentRequest(assessment))
  });

  return result ? normalizeHrAssessment(result) : null;
}
