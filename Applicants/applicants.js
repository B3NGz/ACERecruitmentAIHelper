...
function renderApplicants() {
  ...
  list.innerHTML = itemsToShow.map(a => {
    ...
    return `<tr data-applicant-id="${a.id}">
      ...
      <td><a href="https://b3ngz.github.io/Applicants/applicant-details.html?id=${a.id}" class="btn btn-secondary" style="padding:0.3rem 1rem;">View</a></td>
    </tr>`;
  }).join('');
      ...
      if (trashedApplicants.length === 0) {
        ...
        listContainer.innerHTML = trashedApplicants.map(a => `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:0.5rem 0;border-bottom:1px solid var(--border-light);">
            <span><a href="https://b3ngz.github.io/Applicants/applicant-details.html?id=${a.id}" style="color:var(--link-color);text-decoration:none;">${a.fullName}</a></span>
